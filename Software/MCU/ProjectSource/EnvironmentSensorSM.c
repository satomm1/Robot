...
ES_Event_t RunEnvironmentSensorSM(ES_Event_t ThisEvent)
{
  ...
  switch (CurrentState)
  {
    case InitPState_Env:        // If current state is initial Psedudo State
    {
      if (ThisEvent.EventType == ES_INIT)    // only respond to ES_Init
      {
        ...
        CurrentState = EnvReset;
      }
    }
    break;

    case EnvReset:        // If current state is state one
    {
      switch (ThisEvent.EventType)
      {
        ...
        {   // Execute action function for state one : event one
          CurrentState = EnvReset;  //Decide what the next state will be
        }
        break;
