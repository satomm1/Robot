...
The GUI Robot Startup panel (in [dds_robot_platform](https://github.com/satomm1/dds_robot_platform)) talks to a small HTTP service on the **Jetson base machine** (not inside Docker) for Docker start/stop and power off. ROS launch remains on port **8080** inside the `ros_noetic` container (`startup_script.py`).

You do **not** need to clone the GUI repo on the Jetson. Copy one install script from this repo and run it.

## Install

From your development machine (paths relative to this Robot repo):
