...
/* Rate constants for programming the SysTick Period to generate tick interrupts.
   These assume that we are using the M4K core timer running at 60MHz. Even
   thought the processor clock is 120MHz the core timer increments every other 
   clock edge, resulting in a divide by 2 from the instruction clock.
 */
typedef enum
{
  ES_Timer_RATE_OFF   = (0),
  ES_Timer_RATE_d5mS   = 30000,     /* 0.5ms timer tick */  
  ES_Timer_RATE_1mS  = 60000,       /* 1ms timer tick */
  ES_Timer_RATE_2mS  = 120000,       /* 2ms timer tick */
  ES_Timer_RATE_4mS  = 240000,       /* 4ms timer tick */
  ES_Timer_RATE_5mS  = 300000,       /* 5ms timer tick */
}TimerRate_t;
